/**
 * CI preflight for Growth Ops review-watch workflow.
 * Prints yes/no configuration labels only. Never prints secret values.
 *
 * Exit 0 when required GBP/OAuth config is present.
...
import {
  ...
  formatReviewWatchConfigSummary,
...
if (isMain) {
  try {
    ...
    if (options.help) {
      console.log(
        "Usage: node scripts/growth-review-watch-preflight.mjs [--summary-out path] [--json-out path]"
      );
      process.exit(0);
    }
    const assessment = assessReviewWatchConfig(process.env);
    const summary = formatReviewWatchConfigSummary(assessment);
